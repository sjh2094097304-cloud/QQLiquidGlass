...
本副本已按用户要求取消 `SignatureGuard` 的应用内原证书限制，Android 安装签名仍然保留。与上一份修复源码相比，其他应用代码、头像、资源及包名未变。用测试签名可以构建私人 APK，但换签名不能覆盖原版。

私人构建请看 [PRIVATE_BUILD.md](PRIVATE_BUILD.md)，运行 `python3 tools/build_private.py --sdk /实际路径/Android/Sdk`。它不要求原密钥。下方保留的是原 3.7 修复记录和原证书发布通道，`build_release.py` 仍会检查原证书。
